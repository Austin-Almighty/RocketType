"use client";
import { motion, useAnimate } from "motion/react";
import { useEffect } from "react";

export default function Campfire() {
  const [flameRef, animate] = useAnimate();

  useEffect(() => {
    let isMounted = true;

    function flicker() {
      if (!isMounted) return;

      const scale = 1 + (Math.random() * 0.1 - 0.05);
      const skewX = Math.random() * 10 - 5;
      const opacity = 0.7 + Math.random() * 0.3;
      const fillColors = ["#EBBA16", "#F0C419", "#D9A715"];
      const fill = fillColors[Math.floor(Math.random() * fillColors.length)];

      animate(
        flameRef.current,
        {
          scale: scale,
          skewX: skewX,
          opacity: opacity,
          fill: fill,
        },
        { duration: 0.1 + Math.random() * 0.2 }
      );

      setTimeout(flicker, 100 + Math.random() * 200);
    }

    flicker();
    return () => {
      isMounted = false;
    };
  }, [animate, flameRef]);

  return (
    <>
      <motion.svg
        height="80px"
        width="80px"
        version="1.1"
        id="Capa_1"
        viewBox="0 0 55.243 55.243"
      >
        <motion.path
          fill="#6C4127"
          d="M52.259,45.386l-15.06-5.394c0.031-0.273,0.033-0.549,0.016-0.827
            c-2.727,2.476-6.078,3.956-9.715,3.956c-3.532,0-6.797-1.391-9.481-3.739c-0.003,0.204,0.003,0.408,0.025,0.61L2.984,45.386
            c-2.33,0.835-3.554,3.424-2.719,5.754s3.424,3.554,5.754,2.719l21.603-7.737l21.603,7.737c2.33,0.834,4.919-0.389,5.754-2.719
            C55.813,48.81,54.589,46.221,52.259,45.386z"
        />
        <motion.circle fill="#492C1C" cx="4.501" cy="49.62" r="4.5" />
        <motion.circle fill="#492C1C" cx="50.742" cy="49.62" r="4.5" />
        <motion.path
          fill="#895D45"
          d="M16.949,46.805c-0.175-0.523-0.74-0.808-1.265-0.632l-6.858,2.286
            c0.189,0.702,0.208,1.412,0.065,2.087l7.425-2.475C16.84,47.896,17.124,47.329,16.949,46.805z"
        />
        <motion.path
          fill="#895D45"
          d="M12.949,44.805c-0.174-0.524-0.739-0.809-1.265-0.632l-4.857,1.619
            c0.635,0.385,1.18,0.916,1.566,1.586l3.924-1.308C12.84,45.896,13.124,45.329,12.949,44.805z"
        />
        <motion.path
          fill="#895D45"
          d="M48.329,45.844l-5.013-1.671c-0.525-0.176-1.09,0.108-1.265,0.632s0.108,1.09,0.632,1.265
            l4.134,1.378C47.188,46.778,47.711,46.238,48.329,45.844z"
        />
        <motion.path
          fill="#895D45"
          d="M46.399,48.534l-7.082-2.361c-0.526-0.175-1.091,0.109-1.265,0.632
            c-0.175,0.524,0.108,1.09,0.632,1.265l7.682,2.561C46.209,49.956,46.222,49.241,46.399,48.534z"
        />
        <motion.path
          fill="#BF8840"
          d="M37.199,39.993c0.031-0.273,0.033-0.549,0.016-0.827c-2.727,2.476-6.078,3.956-9.715,3.956
            c-3.532,0-6.797-1.391-9.481-3.739c-0.003,0.204,0.003,0.408,0.025,0.61l-4.261,1.526c2.108,3.042,4.967,5.365,8.264,6.6
            l5.574-1.996l5.443,1.949c3.274-1.256,6.11-3.583,8.198-6.624L37.199,39.993z"
        />
        <g>
          <motion.path
            ref={flameRef}
            fill="#EBBA16"
            d="M41,8.122c0,9.5-8,16-8,16c0-7.5-3-14-3-14c0,5.875-4,9-4,9c-5.344-5.344,0-18,0-18
                c-7.61,1.004-14.425,6.25-14.425,15c0,6.188,1.728,10.304,4.425,13c0,0-2.393-1.388-4.78-2.304
                c1.238,9.8,8.044,17.304,16.28,17.304c9.113,0,16.5-8.438,16.5-20S41,8.122,41,8.122z"
          />
        </g>
        <motion.path
          fill="#DC691D"
          d="M32.912,30.564c0,3.314-3.535,6.279-3.535,6.279c0-2.616-1.326-5.233-1.326-5.233
            c0,2.049-3.093,2.093-3.093,2.093c-1.381-1.483,0-5.581,0-5.581c-3.363,0.35-5.048,2.18-5.048,5.233
            c0,2.158,0.764,3.594,1.955,4.535,c0,0-1.057-0.484-2.112-0.804c0.547,3.418,3.555,6.036,7.193,6.036
            c4.027,0,7.291-2.943,7.291-6.977S32.912,30.564,32.912,30.564z"
        />
      </motion.svg>
    </>
  );
}
